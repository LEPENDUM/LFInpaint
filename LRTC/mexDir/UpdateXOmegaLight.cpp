...
void checkInputs(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	// Check lambda
	if (mxGetClassID(prhs[3]) != mxSINGLE_CLASS || mxGetNumberOfElements(prhs[3]) != 1)
		mexErrMsgTxt("lambda parameter should be a scalar of type single.");
...
//Inputs:
	...
	float *M = (float*)mxGetData(prhs[1]);
	...
	cIndices = (unsigned int*)mxGetData(prhs[4]);
